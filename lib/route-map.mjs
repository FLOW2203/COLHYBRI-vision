// Single source of truth for locale-specific URL slugs.
// Consumed by lib/navigation.ts (sitemap, hreflang, links) AND next.config.mjs
// (localized-slug rewrites/redirects), so routing and the sitemap never drift.
// Plain .mjs so it can be imported by both the TypeScript app and the ESM config.
