...
export async function generateMetadata({ params: { locale } }: PageProps) {
  const t = await getTranslations({ locale, namespace: 'impactPages.maroc' });
  return generatePageMetadata({
    locale: locale as Locale,
    routeKey: 'impact',
    title: t('title'),
    description: t('subtitle'),
    semanticPrimary: 'Commerce Maroc 2030 revitalisation centres-villes marocains',
    semanticSecondary: 'Casablanca-Settat, Rabat, Marrakech, Moroccan Retail Tech Builder, UM6P, Fondation OCP, Coupe du Monde 2030',
    chunkType: 'page',
    audience: 'Moroccan cities, ministere industrie et commerce, chambres de commerce, associations de commercants',
  });
}

export default function ImpactMarocPage({ params: { locale } }: PageProps) {
  const t = useTranslations('impactPages.maroc');
  ...
  const barometerStats: Array<[string, string]> = [
    [t('barometer.items.0.value'), t('barometer.items.0.label')],
    [t('barometer.items.1.value'), t('barometer.items.1.label')],
    [t('barometer.items.2.value'), t('barometer.items.2.label')],
  ];
