...
export async function generateMetadata({ params: { locale } }: PageProps) {
  const t = await getTranslations({ locale, namespace: 'impactPages.uk' });
  return generatePageMetadata({
    locale: locale as Locale,
    routeKey: 'impact',
    title: t('title'),
    description: t('subtitle'),
    semanticPrimary: 'UK high street revitalization Institute of Place Management',
    semanticSecondary: 'High Streets Task Force, Plan for Neighbourhoods, HSRAs, BIDs, Manchester, London',
    chunkType: 'page',
    audience: 'UK local authorities, BIDs, HSTF alumni, placemaking leaders',
  });
}

export default function ImpactUkPage({ params: { locale } }: PageProps) {
  const t = useTranslations('impactPages.uk');
