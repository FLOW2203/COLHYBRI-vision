...
export async function generateMetadata({ params: { locale } }: PageProps) {
  const t = await getTranslations({ locale, namespace: 'impactPages.espana' });
  return generatePageMetadata({
    locale: locale as Locale,
    routeKey: 'impact',
    title: t('title'),
    description: t('subtitle'),
    semanticPrimary: 'comercio de proximidad Barcelona Capital Europea 2026',
    semanticSecondary: 'Eix Sarria, Poblenou, Cataluna, BIDs, Vitrines d Europe, EISMEA',
    chunkType: 'page',
    audience: 'Spanish cities, Ajuntaments, asociaciones de comerciantes, BIDs',
  });
}

export default function ImpactEspanaPage({ params: { locale } }: PageProps) {
  const t = useTranslations('impactPages.espana');
