...
export async function generateMetadata({ params: { locale } }: PageProps) {
  const titles = {"fr":"IDB et commerce local en Amerique Latine","en":"IDB and Local Commerce in Latin America","es":"BID y comercio local en America Latina"};
  const descs = {"fr":"Amerique Latine : 2.1% croissance, 55% informel. IDB + COLHYBRI = formalisation digitale.","en":"Latin America: 2.1% growth, 55% informal. IDB + COLHYBRI = digital formalization.","es":"America Latina: 2.1% crecimiento, 55% informal."};
  return {
    ...
    <main className="min-h-screen bg-colhybri-cream">
      <section className="max-w-4xl mx-auto px-4 py-16">
        <nav className="text-sm text-colhybri-dark/60 mb-8 font-sans">
          ...
          {' > '}
          <Link href={`/${locale}/impact/latam`} className="hover:text-colhybri-teal">impact/latam</Link>
          {' > '}
          <span className="text-colhybri-dark">{t('idb-latin-america.title')}</span>
        </nav>
        <h1 className="font-display text-4xl sm:text-5xl font-semibold text-colhybri-dark mb-6 leading-[1.1]">
          {t('idb-latin-america.title')}
        </h1>
        <p className="font-sans text-lg text-colhybri-dark/80 leading-relaxed mb-12">
          {t('idb-latin-america.desc')}
        </p>
        <section className="bg-white rounded-2xl p-8 shadow-sm border border-colhybri-teal/10 mb-12">
          ...
          <div className="flex flex-wrap gap-4 font-sans">
            <Link href={`/${locale}/impact/latam`} className="px-6 py-3 bg-colhybri-teal text-white rounded-full hover:bg-colhybri-gold transition-all">
              Retour au dossier
            </Link>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "Article",
        "headline": "IDB and Local Commerce in Latin America",
